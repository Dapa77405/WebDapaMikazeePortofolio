...
// Demo Calculator - modern mini calculator
const DemoCalculator = () => {
  ...
  const applyOp = (op) => {
    try {
      // Use Function for simple eval of current expression - safe here for demo only
      // Build expression by appending operator
  ...
  return (
    <div className="mx-auto max-w-md">
      <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
        <div className="mb-4 text-right text-3xl font-mono font-semibold break-words">{value}</div>
          ...
          {buttons.flat().map((b, i) => {
            ...
            return (
              <button key={i} onClick={()=>{
                ...
                inputDigit(b)
              }} className={`rounded-lg py-3 px-2 text-lg font-semibold ${isOp ? 'bg-gradient-to-r from-indigo-500 to-pink-500 text-white' : 'bg-white/3 text-white'}`} style={{ gridColumn: `span ${colSpan}` }}>
                {b}
              </button>
            )
          })}
